...
We study the time-dependent Hartree-Fock-Boboliubov (TDHFB) theory
...
associated with low-eneryg
large amplitude collective motion,
which leads to difficutlties in description of the quantum tunneling
...
relative angle $\phi$.
Here, the ``rotational behavior'' means that the motion span the whole
...
% \begin{center}
%%  \includegraphics[bb=0 0 120 120]{images/Sommerfeld1.png}
% \end{center}
% \end{minipage}
% \begin{minipage}{0.5\hsize}
% \begin{center}
%%  \includegraphics[bb=0 0 120 120]{images/Sommerfeld2.png}
% \end{center}
% \end{minipage}
...
%pairing vibrational mode can be obtained by solving the Schrodinger equation
\begin{equation}
	H\left( \phi,-i\frac{d}{d\phi};\frac{N}{2} \right)
	\psi_k^{(N)}(\phi) = E_k^{(N)}\psi_k^{(N)}(\phi),
	\label{Schroedinger_eq}
\end{equation}
%where $i$ is the excitation index under the system of fixed total particle number $N$. To solve the Schrodinger equation, the wave function can be expanded as
...
We choose Neumann boundary condition $u_k^{(N)}(q)'|_{q=q_{min}}=0$, 
...
(Sec.~\ref{subsec:canonical1}) as ``CQ''.
...
\begin{figure}[t]
 \begin{center}
%  \includegraphics[width=60mm,angle=-90]{images/N50ex_energy_wo_adiabatic.eps}
%  \includegraphics[width=60mm,angle=-90]{images/N100ex_energy_wo_adiabatic.eps}
	 (a)  \includegraphics[height=0.46\textwidth,angle=-90]{images/N50ex_energy_wo_adiabatic.eps}
	 (b)  \includegraphics[height=0.46\textwidth,angle=-90]{images/N100ex_energy_wo_adiabatic.eps}
 \end{center}
...
\begin{figure}[t]
 ...
 \begin{center}
%  \includegraphics[width=67mm,angle=-90]{images/N50Xeq0p5occ_wo_adiabatic.eps}
%  \includegraphics[width=67mm,angle=-90]{images/N100Xeq0p5occ_wo_adiabatic.eps}
...
(b1)\includegraphics[height=0.45\textwidth,angle=-90]{images/N100Xeq0p5occ_wo_adiabatic.eps}
 ...
 \begin{minipage}{1\hsize}
 \begin{center}
%  \includegraphics[width=67mm,angle=-90]{images/N50Xeq2occ_wo_adiabatic.eps}
%  \includegraphics[width=67mm,angle=-90]{images/N100Xeq2occ_wo_adiabatic.eps}
(a2)  \includegraphics[height=0.45\textwidth,angle=-90]{images/N50Xeq2occ_wo_adiabatic.eps}
(b2)  \includegraphics[height=0.45\textwidth,angle=-90]{images/N100Xeq2occ_wo_adiabatic.eps}
 \end{center}
 \end{minipage}
 \caption{
...
with the constant value of $S_+$.
Since the constant $S_+$ provides only $\Delta k=0$ intraband transitions,
...
respectively. 
See the caption of Fig.~\ref{fig:N50_occ}.
...
the interband $k=1\rightarrow 0$ transitions decrease as
functions of $x$.
...
\begin{figure}[t]
 \begin{center}
  \includegraphics[width=60mm,angle=-90]{images/N16ex_energy_wo_adiabatic.eps}
 \end{center}
...
\begin{figure}[thb]
 \begin{minipage}{1\hsize}
 \begin{center}
(a) \includegraphics[height=0.46\textwidth,angle=-90]{images/N16Xeq0p5occ_wo_adiabatic.eps}
(b) \includegraphics[height=0.46\textwidth,angle=-90]{images/N16Xeq2occ_wo_adiabatic.eps}
 \end{center}
 \end{minipage}
...
\begin{align}
\alpha(j,J) = \braket{Z_0|\hat{S}_-|Z_0} %= g\sum_{k>0} u_kv_k \nonumber \\
	= \sum_{l=1}^2 S_l\sqrt{1-q_l^2} . % = \Delta(\pi) .
 \label{Delta_BCS}
\end{align}
$\alpha$ is equivalent to pairing gap $\Delta$ as a coordinate because the gap equation
\begin{align}
...
in Fig. \ref{fig:p_Delta1} ($N=8$) and Fig. \ref{fig:p_Delta2} ($N=16$).
%These results correspond to the system in Section 4.2. 
...
Fig. \ref{fig:Delta_E} shows two examples of the relationship 
between excitation energies and energy surface.
In the left panel ($\Omega=50$, $N=50$, $x=2.4$), the excited $0^+$ states are bound up to second excitation.
For very large degeneracy, the pocket of energy surface is deep, 
hence the low-lying excited states can be described by $\alpha$. 
However, in the right panel ($\Omega=8$, $N=16$, $x=2.4$), 
only the ground state is bound, and all excited $0^+$ states are unbound.
For small degeneracy, the shallow pocket of energy surface allows 
the excited $0^+$ states to be unbound. Therefore, pairing deformation parameter
is not proper collective coordinate in small $\Omega$ and strong pairing case.
...
\begin{figure}[htbp]
 ...
 \begin{center}
  \includegraphics[width=50mm,angle=-90]{images/N50Xeq2p4gap_E.eps}
 \end{center}
 \end{minipage}
 \begin{minipage}{0.45\hsize}
 \begin{center}
  \includegraphics[width=50mm,angle=-90]{images/N16Xeq2p4gap_E.eps}
 \end{center}
\end{minipage}
 \caption{Energy surface with respect to $\alpha$ in $x=2.4$. 
Green lines are energy spectra until 3rd excitation obtained from CQ 
(Sec.~\ref{sec:canonical}). Left panels: $\Omega=50$, $N=50$ system;
...
However, the pairing collectivity may not be too weak to justify this
treatment.
...
%\begin{align}
 % \sum_{m=0}^{J}{}_J C_m\int_0^T dt \exp{\left( i\int^t \pi(t') \dot{\phi}(t') dt'-i(J/2-m)\phi(t) \right)}A(q_1,S_1,m)A(q_2,S_2,J-m) (S_1^{\dag})^m(S_2^{\dag})^{J-m}\ket{0}
%\end{align}
