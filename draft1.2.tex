...
\begin{align}
  S_+(\Phi,J;\phi,j) &= \braket{Z|\hat{S}_+|Z}
 = \frac{1}{2}
...
The deformation parameter $\alpha\equiv \braket{ \hat{S}_- }$
in the gauge space is
usually taken as real value ($\Phi=0$).
...
%	= \sum_{l=1}^2 S_l\sqrt{1-q_l^2} . % = \Delta(\pi) .
 ...
 \label{Delta_BCS}
\end{align}
The parameter $\alpha$ is equivalent to pairing gap $\Delta$,
...
\begin{figure}[tb]
 \begin{center}
(a) \includegraphics[height=0.46\textwidth,angle=-90]{images/N8p_E.eps}
(b) \includegraphics[height=0.46\textwidth,angle=-90]{images/N16p_E.eps}
 \end{center}
 \caption{Energy surface as functions of $j$
 for $x=0.5$, $x=2$ and $x=3.2$.
...
in the $j<0$ region.
The potential energy, $E=\mathcal{H}(\phi=0,j;J)$, is also shown in
Fig.~\ref{fig:p_Delta}.
...
at the boundary point ($j=0$) with the maximum value of $\alpha$ ($\Delta$).
...
% \begin{minipage}{0.45\hsize}
 \begin{center}
(a)  \includegraphics[height=0.46\textwidth,angle=-90]{images/N50Xeq2p4gap_E.eps}
% \end{center}
% \end{minipage}
% \begin{minipage}{0.45\hsize}
% \begin{center}
(b)  \includegraphics[height=0.46\textwidth,angle=-90]{images/N16Xeq2p4gap_E.eps}
 \end{center}
%\end{minipage}
\caption{Potential energy surface as functions of $\alpha$ with $x=2.4$;
(a) $\Omega=N=50$,
(b) $2\Omega=N=16$.
Horizontal (green) lines indicate
 energy spectra obtained with the CQ method
