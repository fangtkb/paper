...
the canonical quantization often fails to produce an approximate value %answer
...
\begin{eqnarray}
        S_\alpha^0 &=& \frac{1}{2}\left(\sum_ma_{j_\alpha m}^{\dag}a_{j_\alpha m}-\Omega_\alpha\right) ,\\
...
\end{equation}
where $\hat{N}=\sum_\alpha n_\alpha$.
...
($\delta q = 10dq =0.1/\sqrt{\epsilon_2-\epsilon_1}$). 
...
are $2\sim3.5$ MeV.
{\bf For $^{186}$Pb, the height of the potential is not
...
{\bf See the potentials in Figs. \ref{potential} and \ref{Pb_potential}.
