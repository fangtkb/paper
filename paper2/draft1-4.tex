...
which indicates $f^n_{,\gamma\beta}$ in Eq.~(\ref{M}) is zero. 
...
variables $(q^1,p_1)$ are not unique, because it can be arbitrarily mixed
with the pair rotation $(q^n,p_n)$ as
\begin{equation}
	q^1 \rightarrow q^1 + c q^n,  \quad
	p_n \rightarrow p_n -c p_1
\end{equation}
with an arbitrary constant $c$.
Numerically, this arbitrariness sometimes leads to a problematic bahavior 
...
The left end of the horizontal axis at $m_2-m_1=-8$
corresponds to a state with $(m_1,m_2)=(N/2,0)$ where all the particles
are in the lower level $\alpha=1$.
The next at $m_2-m_1=-6$ corresponds to the one with
$(m_1,m_2)=((N-2)/2,1)$, and so on.
The results from TDHFB2+SPA and ASCC+SPA are identical to each other
...
of ASCC+SPA, TDHFB2+SPA, and TDHFB+SPA, respectively. 
...
We calculate the action integrals on each trajectory
...
ASCC+SPA, TDHFB2+SPA and TDHFB+SPA calculations,
...
\begin{figure*}[t]
 ...
 \end{center}
\caption{Eigenvalues of moving-frame QRPA equation with respect
to the collective coordinate $q^1$, from $N=14$ to $N=24$.
...
with respect to the collective coordinate $q$,
from $N=14$ to $N=24$.
The purple, green, and blue lines correpsond to
...
\begin{figure}[htbp]
 ...
 \end{center}
	\caption{Collective potential $V(q^1)$ obtained from ASCC.
...
This work is supported in part by JSPS KAKENHI Grants No. 18H01209
and
